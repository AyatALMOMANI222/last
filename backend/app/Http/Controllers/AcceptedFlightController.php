...
class AcceptedFlightController extends Controller
{
    ...
    public function storeAll(Request $request)
    {
        try {
            ...
            $acceptedFlights = [];
            $isOtherFound = false; // Variable to check if isOther is true in any flight
            $userId = Auth::id();
            $userName = Auth::user()->name;

            // Iterate through the validated data and handle each flight
            foreach ($validatedData['flights'] as $flightData) {
                $invoiceFlight = InvoiceFlight::where('flight_id', $flightData['flight_id'])->first();
                if (isset($flightData['isOther']) && $flightData['isOther'] === 1) {
                    $isOtherFound = true; // At least one isOther is true
                }
                if ($invoiceFlight) {
                    // Check the status of the invoice_flight
                    if ($invoiceFlight->status === 'approved') {
                        return response()->json([
                            'error' => "Cannot edit flight ID {$flightData['flight_id']} because the payment is done.",
                        ], 200);
                    } elseif ($invoiceFlight->status === 'pending') {
                        // Allow editing: Delete old AcceptedFlight record if exists
                        AcceptedFlight::where('flight_id', $flightData['flight_id'])->delete();
                    }
                }

                // Create a new AcceptedFlight record
                $acceptedFlight = AcceptedFlight::create([
                    'flight_id' => $flightData['flight_id'],
                    'price' => $flightData['price'],
                    'departure_date' => $flightData['departure_date'] ?? null,
            ...
            if ($isOtherFound) {
                ...
                foreach ($admins as $admin) {
                    ...
                    $notification = Notification::create([
                        ...
                        'message' => "The user {$userName} has requested additional trips. Please add them accordingly.",
                        'is_read' => false,
                    ]);
