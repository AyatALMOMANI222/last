...
const handleViewAirportBooking = (conferenceId) => {
    ...
    useEffect(() => {
        const fetchConferences = async () => {
            ...
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/con/upcoming', {
                    headers: {
                        ...
                        <tbody>
                            {conferences?.map(conference => (
                                <tr key={conference.id}>
                                    ...
                                    <td>{new Date(conference.start_date).toLocaleDateString('en-GB')}</td>
                                    <td>{new Date(conference.end_date).toLocaleDateString('en-GB')}</td>
                                    <td>{conference.location}</td>
                                    <td>
                                        <button className="submit-btn" onClick={() => handleViewUserDinner(conference.id)}>
                                            View User Dinner
                                        </button>
                                        <button className="submit-btn" onClick={() => handleViewAirportBooking(conference.id)}>
