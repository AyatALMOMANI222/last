...
const AddDiscountForm = ({ isOpen, setIsOpen, userId }) => {
  ...
  const fetchUserConferences = async () => {
    try {
      const response = await httpService({
        method: "GET",
        url: `http://127.0.0.1:8000/api/user/${userId}/conferences`,
  ...
  const fetchConferenceTrips = async (conferenceId) => {
    ...
    try {
      const response = await httpService({
        method: "GET",
        url: "http://127.0.0.1:8000/api/conference-trips",
        headers: { Authorization: `Bearer ${getAuthToken()}` },
      });
  ...
  const fetchTripOptions = async (tripId) => {
    ...
    try {
      const response = await httpService({
        method: "GET",
        url: `http://127.0.0.1:8000/api/additional-options/trip/${tripId}`,
        headers: { Authorization: `Bearer ${getAuthToken()}` },
      });
